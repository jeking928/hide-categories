  ...
  if (zen_not_null($action)) {
    switch ($action) {
	  ...
	  if (isset($_POST['category_hide'])) {
	  	$hide_status = $db->Execute("select * FROM " . TABLE_HIDE_CATEGORIES . " WHERE categories_id = " . $_POST['categories_id']);
		if ($hide_status->RecordCount() < 1) {
		  	zen_db_perform(TABLE_HIDE_CATEGORIES, array('categories_id' => zen_db_prepare_input($_POST['categories_id']), 'visibility_status' => zen_db_prepare_input($_POST['category_hide'])));
		} else {
			zen_db_perform(TABLE_HIDE_CATEGORIES, array('categories_id' => zen_db_prepare_input($_POST['categories_id']), 'visibility_status' => zen_db_prepare_input($_POST['category_hide'])), 'update', "categories_id = '" . $_POST['categories_id'] . "'");
		}
	  }
...
<?php
  ...
  switch ($action) {
	...
	$contents[] = array('text' => '<br />' . TEXT_EDIT_HIDE_STATUS . ' ' . zen_draw_radio_field('category_hide', '0', $hide_category_normal) . '&nbsp;' . TEXT_EDIT_HIDE_NORMAL . ' ' . zen_draw_radio_field('category_hide', '1', $hide_category_nomenu) . '&nbsp;' . TEXT_EDIT_HIDE_NOMENU . ' ' . zen_draw_radio_field('category_hide', '2', $hide_category_hidden) . '&nbsp;' . TEXT_EDIT_HIDE_HIDDEN);
	//End hideCategories code
