...
<?php if ($action == '') { ?>
				...
				$hide_status = $db->Execute("select visibility_status 
										...
										LIMIT 1");
				if ($hide_status->fields['visibility_status'] == 0) $folder_gif = '_NORMAL';
				if ($hide_status->fields['visibility_status'] == 1) $folder_gif = '_NOMENU';
				if ($hide_status->fields['visibility_status'] == 2) $folder_gif = '_HIDDEN';
				$folder_gif_alt = "ICON_FOLDER" . $folder_gif;
				eval("\$folder_gif_alt = $folder_gif_alt;");
				echo '<td class="dataTableContent"><a href="' . zen_href_link(FILENAME_CATEGORIES, zen_get_path($categories->fields['categories_id'])) . '">' . zen_image(DIR_WS_ICONS . 'folder' . $folder_gif . '.gif', $folder_gif_alt) . '</a>&nbsp;<b>' . $categories->fields['categories_name'] . '</b></td>';
